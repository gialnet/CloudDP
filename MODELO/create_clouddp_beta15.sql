...
CREATE SEQUENCE S_WEB_PERSONAL
    START WITH 1
    INCREMENT BY 1
    MINVALUE 1
    NOMAXVALUE
...
CREATE SEQUENCE S_CARPETASUSUARIO
    START WITH 1
    INCREMENT BY 1
    MINVALUE 1
    NOMAXVALUE
...
COMMENT ON COLUMN USUARIOS.USUARIO IS 'EL NOMBRE DE USUARIO';

COMMENT ON COLUMN USUARIOS.IDCLIENTE IS 'id de la empresa  a la que pertenence';
...
CREATE TABLE PERFILES_DIGITALIZACION (
    ID NUMBER(38) CONSTRAINT PRFLS_DGTLZCN_ID NOT NULL,
    DESCRIPCION VARCHAR2(256),
    CERTIFICADO NUMBER(38),
    NUMERO_ESCANEOS NUMBER(38),
    CLIENTE INTEGER,
    CONSTRAINT PK_PRFLS_DGTLZCN PRIMARY KEY (ID)
);
...
CREATE TABLE LOTES_CARGA (
    ID INTEGER CONSTRAINT LTS_CRG_ID NOT NULL,
    FECHA TIMESTAMP,
    ...
    ESTADO CHAR(2),
    F_FIN TIMESTAMP,
    CONSTRAINT PK_LTS_CRG PRIMARY KEY (ID)
);
...
COMMENT ON COLUMN Docs_Metadatos.VALOR IS 'EN CASO DE SER FACTURA AQUI SE ESPECIFICARÁ EL PORCENTAJE DE IVA';
...
/* ---------------------------------------------------------------------- */
/* Add table "FormasJuridicas"                                            */
/* ---------------------------------------------------------------------- */

CREATE TABLE FormasJuridicas (
    FORMA VARCHAR2(40),
    IDTIPO_DOC INTEGER
);
...
CREATE TABLE Consultas (
    ID INTEGER CONSTRAINT CNSLTS_ID NOT NULL,
    NOMBRE VARCHAR2(40),
    IDGREMIO INTEGER CONSTRAINT CNSLTS_IDGREMIO NOT NULL,
    CONSTRAINT PK_CNSLTS PRIMARY KEY (ID)
);

/* ---------------------------------------------------------------------- */
/* Add table "DocConsulta"                                                */
/* ---------------------------------------------------------------------- */

CREATE TABLE DocConsulta (
    ID_CONSULTA INTEGER CONSTRAINT DCCNSLT_ID_CONSULTA NOT NULL,
    TIPO_DOC VARCHAR2(40)
);

/* ---------------------------------------------------------------------- */
/* Add table "TempConsultas"                                              */
/* ---------------------------------------------------------------------- */

CREATE TABLE TempConsultas (
    ID_DOC INTEGER CONSTRAINT TMPCNSLTS_ID_DOC NOT NULL,
    SESION INTEGER CONSTRAINT TMPCNSLTS_SESION NOT NULL,
    TIPO CHAR(2)
);

COMMENT ON COLUMN TempConsultas.TIPO IS 'EMITIDAS - EM RECIBIDAS - RE INTRACOMUNITARIAS EMITIDAS - IE INTRACOM RECIBIDAS - IR';

/* ---------------------------------------------------------------------- */
/* Add table "TmpResultados"                                              */
/* ---------------------------------------------------------------------- */

CREATE TABLE TmpResultados (
    SESION INTEGER CONSTRAINT TMPRSLTDS_SESION NOT NULL,
    IVA NUMBER(4,2),
    BASE NUMBER(10,3),
    NUMFACTURAS INTEGER,
    TIPOFACTURA CHAR(2)
...
CREATE TABLE TIPOS_DOC (
    ID INTEGER CONSTRAINT TPS_DC_ID NOT NULL,
    ETIQUETA VARCHAR2(40),
    IDGREMIO INTEGER CONSTRAINT TPS_DC_IDGREMIO NOT NULL,
    DESCRIPCION VARCHAR2(90),
    PERIODO CHAR(2) DEFAULT 'TR',
    TIPO_DOCUMENTO VARCHAR2(90),
    CONSTRAINT PK_TPS_DC PRIMARY KEY (ID)
);

COMMENT ON COLUMN TIPOS_DOC.ETIQUETA IS 'ESTE CAMPO ES EL QUE SE VISUALIZA EN EL INTERFAZ';

COMMENT ON COLUMN TIPOS_DOC.TIPO_DOCUMENTO IS 'ESTE CAMPO ES IGUAL QUE EL DE ETIQUETA PERO EL QUE SE UTILIZA INTERNAMENTE PARA LAS BUSQUEDAS ES EL CAMPO TIPO_DOCUMENTO EN DOC_CUSTODIA';

/* ---------------------------------------------------------------------- */
/* Add table "CALENDARIO_DOCS"                                            */
/* ---------------------------------------------------------------------- */

CREATE TABLE CALENDARIO_DOCS (
    IDTIPODOC INTEGER CONSTRAINT CLDNDR_DCS_IDTIPODOC NOT NULL,
    FECHA DATE
);

/* ---------------------------------------------------------------------- */
/* Add table "WEB_PERSONAL"                                               */
/* ---------------------------------------------------------------------- */

CREATE TABLE WEB_PERSONAL (
    IDCLIENTE INTEGER CONSTRAINT WB_PRSNL_IDCLIENTE NOT NULL,
    URL VARCHAR2(250),
    TIPO CHAR(2),
    FECHA_ALTA DATE DEFAULT SYSTIMESTAMP,
    FECHA_BAJA DATE,
    CONSTRAINT PK_WB_PRSNL PRIMARY KEY (IDCLIENTE)
);

COMMENT ON COLUMN WEB_PERSONAL.TIPO IS 'SI LA WEB ES PERSONALIZADA ''PE'' O ESTANDAR ''ES''';
...
/* ---------------------------------------------------------------------- */
/* Add table "CarpetasUsuario"                                            */
/* ---------------------------------------------------------------------- */

CREATE TABLE CarpetasUsuario (
    ID INTEGER,
    USUARIO VARCHAR2(30),
    NOMBRE VARCHAR2(20)
);
...
/* ---------------------------------------------------------------------- */
/* Add table "temp_carpetas"                                              */
/* ---------------------------------------------------------------------- */


CREATE TABLE temp_carpetas (
    sesion INTEGER,
    id_carpeta INTEGER,
    tipo CHAR(2),
    etiqueta VARCHAR2(40),
...
COMMENT ON COLUMN temp_carpetas.tipo IS 'si es fija fi si es temporal te';
...
-- DOC_CUSTODIA
--
ALTER TABLE DOC_CUSTODIA ADD CONSTRAINT DOC_CUSTODIA_CLIENTES
    FOREIGN KEY (CLIENTE) REFERENCES CLIENTES (ID);
    
ALTER TABLE DOC_CUSTODIA ADD CONSTRAINT DOC_CUSTODIA_EMPLEADOS
    FOREIGN KEY (EMPLEADO) REFERENCES CLIENTES (ID);
    
ALTER TABLE DOC_CUSTODIA ADD CONSTRAINT DOC_CUSTODIA_INVITADOS
    FOREIGN KEY (INVITADO) REFERENCES INVITADOS (ID);
    
ALTER TABLE DOC_CUSTODIA ADD CONSTRAINT DOC_CUSTODIA_ALMACENCERT
    FOREIGN KEY (ALMACENCERT) REFERENCES ALMACENCERT (ID);

ALTER TABLE DOC_CUSTODIA ADD CONSTRAINT DOC_CUSTODIA_LOTES_CARGA
    FOREIGN KEY (LOTE) REFERENCES LOTES_CARGA (ID);
    
    
-- Docs_Metadatos
ALTER TABLE Docs_Metadatos ADD CONSTRAINT Docs_Metadatos_DOC_CUSTODIA 
    FOREIGN KEY (ID_DOC) REFERENCES DOC_CUSTODIA (ID);
    
-- ALMACENCERT
ALTER TABLE ALMACENCERT ADD CONSTRAINT ALMACENCERT_USOS_CERTIFICADO 
    FOREIGN KEY (USO_CERTIFICADO) REFERENCES  USOS_CERTIFICADO (ID);
    
--
-- USUARIOS
--    
ALTER TABLE USUARIOS ADD CONSTRAINT USUARIOS_GREMIOS
    FOREIGN KEY (IDGREMIO) REFERENCES GREMIOS (ID);
    
   
--
-- GRUPOS_DTOS
--    
ALTER TABLE GRUPOS_DTOS ADD CONSTRAINT GRUPOS_DTOS_GRUPOS 
    FOREIGN KEY (ID_GRUPO) REFERENCES GRUPOS (ID_GRUPO);
    
ALTER TABLE GRUPOS_DTOS ADD CONSTRAINT GRUPOS_DTOS_DEPARTAMENTOS 
    FOREIGN KEY (ID_DTO) REFERENCES DEPARTAMENTOS (ID_DTO);
    
ALTER TABLE GRUPOS_DTOS ADD CONSTRAINT GRUPOS_DTOS_CLIENTES
    FOREIGN KEY (IDCLIENTE) REFERENCES CLIENTES (ID);
--  
-- CLIENTES
--
ALTER TABLE CLIENTES ADD CONSTRAINT CLIENTES_GRUPOS
    FOREIGN KEY (GRUPO) REFERENCES GRUPOS (ID_GRUPO);

ALTER TABLE CLIENTES ADD CONSTRAINT CLIENTES_DEPARTAMENTOS
    FOREIGN KEY (DTO) REFERENCES DEPARTAMENTOS (ID_DTO);
    
-- INVITADOS    
ALTER TABLE INVITADOS ADD CONSTRAINT INVITADOS_CLIENTES
    FOREIGN KEY (IDCLIENTE) REFERENCES CLIENTES (ID);
    
--    
-- CONSUMOS
--
ALTER TABLE CONSUMOS ADD CONSTRAINT CONSUMOS_SERVICIOS 
    FOREIGN KEY (ID_SERVICIO) REFERENCES SERVICIOS (ID);
    
ALTER TABLE CONSUMOS ADD CONSTRAINT CONSUMOS_CLIENTES
    FOREIGN KEY (ID_CLIENTE) REFERENCES CLIENTES (ID);
    
ALTER TABLE CONSUMOS ADD CONSTRAINT CONSUMOS_WORKFORCE
    FOREIGN KEY (WORKFORCE) REFERENCES WORKFORCE (ID);

ALTER TABLE TRANSACCIONES ADD CONSTRAINT TRANSACCIONES_USUARIOS 
    FOREIGN KEY (USUARIO) REFERENCES USUARIOS (USUARIO);

ALTER TABLE SERVICIOS_CONTRATADOS ADD CONSTRAINT SERVICIOS_CONTRATADOS_CLIENTES
    FOREIGN KEY (ID_CLIENTE) REFERENCES CLIENTES (ID);

--
-- MARIA DEL MAR, REPASAR
--
ALTER TABLE PETICIONES ADD CONSTRAINT PETICIONES_CLIENTES 
    FOREIGN KEY (ID_CLIENTE) REFERENCES CLIENTES (ID);

ALTER TABLE HILOS ADD CONSTRAINT HILOS_PETICIONES 
    FOREIGN KEY (ID_PETICION) REFERENCES PETICIONES (ID);

--
--
--
ALTER TABLE FACTURAS ADD CONSTRAINT FACTURAS_CLIENTES 
    FOREIGN KEY (IDCLIENTE) REFERENCES CLIENTES (ID);
    
ALTER TABLE FISCALIDAD ADD CONSTRAINT FISCALIDAD_FACTURAS 
    FOREIGN KEY (IDFACTURA) REFERENCES FACTURAS (ID);

ALTER TABLE FACTURA_DETALLE ADD CONSTRAINT DETALLE_FACTURAS_FACTURA
    FOREIGN KEY (IDFACTURA) REFERENCES FACTURAS (ID);
...
CREATE TRIGGER Trg_Consultas1
BEFORE INSERT ON CONSULTAS FOR EACH ROW
BEGIN
  SELECT S_CONSULTAS.NEXTVAL
  INTO :NEW.ID
  FROM DUAL;
END;
/

CREATE TRIGGER Trg_TIPOS_DOC1
BEFORE INSERT ON TIPOS_DOC FOR EACH ROW
BEGIN
  SELECT S_TIPOS_DOC.NEXTVAL
  INTO :NEW.ID
  FROM DUAL;
END;
/
...
CREATE TRIGGER Trg_TABLON
BEFORE INSERT ON tablon FOR EACH ROW
BEGIN
  SELECT S_tablon.NEXTVAL
  INTO :NEW.ID
  FROM DUAL;
END;
/

CREATE TRIGGER Trg_CarpetasUsuario1
BEFORE INSERT ON CARPETASUSUARIO FOR EACH ROW
BEGIN
  SELECT s_CARPETASUSUARIO.NEXTVAL
  INTO :NEW.ID
  FROM DUAL;
END;
...
--OPTIMIZACION crear indices para nombre y tipo

create or replace view vw_docsCliente (id,fecha,url_local,visible,firmado,sellado,cifrado,tipo_documento,cliente,ubicacion) as
select id,to_char(fecha,'DD/MM/YYYY hh:mi:ss'),url_local,visible,firmado,sellado,cifrado,tipo_documento,cliente,ubicacion
from doc_custodia 
where invitado is NULL and ubicacion<>'notas_de_voz' order by id desc;
/
...
from doc_custodia order by id desc;
/
