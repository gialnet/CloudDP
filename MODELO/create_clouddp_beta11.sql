...
/* Created on:            2010-10-20 20:50                                */
/* Model version:         Version 2010-10-20 2                            */
/* ---------------------------------------------------------------------- */
...
CREATE INDEX IDX_USUARIOS_1 ON USUARIOS (MOVIL);
...
CREATE TABLE PETICIONES (
    ...
    ESTADO CHAR(8) DEFAULT 'NUEVO' CONSTRAINT PTCNS_ESTADO NOT NULL,
    REMITENTE INTEGER,
    CONSTRAINT PK_PTCNS PRIMARY KEY (ID)
);

COMMENT ON COLUMN PETICIONES.ESTADO IS '''NUEVO'' ->NUEVA PETICION ''LEIDO''->PETICION ANTIGUA RESUELTA->PETICION CERRADA';
...
create or replace view vwDocGuest as
select id,url_local,fecha,invitado from doc_custodia 
where visible='S' and ubicacion='/misdoc/';
